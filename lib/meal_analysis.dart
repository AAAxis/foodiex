...
class ImageService {
  ...
  static Future<MealAnalysisResult> processAndAnalyzeImage(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Debug: Check if healthiness is in the response
        print('[ImageService] 🔍 Checking for healthiness in response...');
        print('[ImageService] 🔍 Response keys: ${responseData.keys.toList()}');
        if (responseData.containsKey('healthiness')) {
          print('[ImageService] ✅ Found healthiness: ${responseData['healthiness']}');
        } else {
          print('[ImageService] ❌ No healthiness field found in response');
        }
        ...
        if (responseData.containsKey('data')) {
          ...
          if (dataSection != null) {
            print('[ImageService] 🔍 Data section keys: ${dataSection.keys.toList()}');
            if (dataSection.containsKey('healthiness')) {
              print('[ImageService] ✅ Found healthiness in data section: ${dataSection['healthiness']}');
            } else {
              print('[ImageService] ❌ No healthiness field found in data section');
            }
          }
        }
  ...
  static double _safeDouble(dynamic value) {
    ...
    print(
      '[ImageService] Unexpected type for double parsing: ${value.runtimeType}, value: $value',
    );
    return 0.0;
  }
  ...
  static DateTime _parseTimestamp(dynamic timestamp) {
    ...
    if (timestamp is num) {
      ...
      final timestampMs = timestamp.toDouble();
      if (timestampMs > 1e12) {
        // Already in milliseconds
        return DateTime.fromMillisecondsSinceEpoch(timestampMs.toInt());
      } else {
        // In seconds, convert to milliseconds
        return DateTime.fromMillisecondsSinceEpoch((timestampMs * 1000).toInt());
      }
    }
    
    print('[ImageService] Unexpected timestamp type: ${timestamp.runtimeType}');
    return DateTime.now();
  }
  ...
  static MealAnalysisResult _parseAnalysisResponse(
    ...
    try {
      ...
      print('[ImageService] 🔍 Full response structure: ${response.keys.toList()}');

      final data = response['data'] ?? response;
      print('[ImageService] 🔍 Data section keys: ${data.keys.toList()}');
      print('[ImageService] 🔍 Raw healthiness value: ${data['healthiness']}');
      print('[ImageService] 🔍 Healthiness type: ${data['healthiness'].runtimeType}');
      ...
      Map<String, String> mealNameMap;
      try {
        mealNameMap = data['mealName'] != null
            ? Map<String, String>.fromEntries(
                (data['mealName'] as Map).entries.map((e) => MapEntry(e.key.toString(), e.value.toString())))
            : {'en': data['mealName']?.toString() ?? 'Unknown Meal'};
      } catch (e) {
        print('[ImageService] Error parsing mealNameMap: $e, data: ${data['mealName']}');
        mealNameMap = {'en': 'Unknown Meal'};
      }

      Map<String, List<String>> ingredientsMap;
      try {
        ingredientsMap = data['ingredients'] != null
            ? Map<String, List<String>>.fromEntries(
                (data['ingredients'] as Map).entries.map((e) => MapEntry(e.key.toString(), List<String>.from(e.value))))
            : {'en': (data['ingredients'] as List<dynamic>? ?? []).map((i) => i.toString()).toList()};
      } catch (e) {
        print('[ImageService] Error parsing ingredientsMap: $e, data: ${data['ingredients']}');
        ingredientsMap = {'en': ['Unknown ingredients']};
      }
